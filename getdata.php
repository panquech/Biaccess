    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        if ($row = mysqli_fetch_assoc($resultl)){
            ...
            if ($row['username'] != "Name"){
                ...
                $sql = "SELECT * FROM users_logs WHERE fingerprint_id=? AND checkindate=? AND timeout='0'";
                $result = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    //Logout
                        ...
                        $sql="UPDATE users_logs SET timeout=? WHERE checkindate=? AND fingerprint_id=? AND timeout='00:00:00'";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
